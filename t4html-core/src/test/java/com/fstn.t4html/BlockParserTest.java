...
public class BlockParserTest {
    ...
    public void simpleModuleCase(){
        ...
        try {
            List<Block> realResult = BlockParser.read().fileEndsWith(Config.BLOCK_EXTENSION).from("src"+File.separator+"test"+File.separator+"resources"+File.separator+"html"+File.separator+"blocks").parse();
    ...
    public void simpleOriginalCase(){
        ...
        try {
            List<Block> realResult = BlockParser.read()
                    .fileEndsWith(".html")
                    .from("src"+File.separator+"test"+File.separator+"resources"+File.separator+"html")
                    .parse();
            Assert.assertEquals("incorrect block parsing",expectedResult,realResult.toString());
        } catch (IOException e) {
            e.printStackTrace();
